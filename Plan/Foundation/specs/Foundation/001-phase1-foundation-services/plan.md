...
**Last Updated**: 2025-11-19 - Added Entra B2B/B2C patterns, token exchange, invitation workflows, and tenant context switching based on LMS WIP Feature 001/002 proven implementations.

## Summary

Establish the foundational infrastructure for NorthStar's microservices architecture by implementing three critical services: **Identity & Authentication** (Microsoft Entra ID B2B/B2C SSO with token exchange pattern + local fallback), **API Gateway** (YARP with routing, rate limiting, circuit breakers), and **Configuration Service** (district/school/calendar management). 

**Key Enhancements from LMS WIP Integration:**
- **Entra B2B/B2C Support**: Explicit support for BOTH B2B (organizational accounts for staff/admins) and B2C (external identities for future students/parents)
- **Token Exchange Pattern (BFF)**: Validates Entra JWT tokens and exchanges for custom LMS tokens with tenant context and cached permissions
- **Tenant-Scoped Roles**: Membership entity tracks user-tenant-role associations, enabling delegated administration
- **Invitation Workflow**: Secure role assignment via email invitation with 7-day expiration and verification status tracking
- **Multi-Tenant Context Switching**: Session-based active tenant tracking for users with access to multiple districts/schools
- **Authorization Caching**: Redis-based caching achieving <50ms authorization decisions with tenant-scoped invalidation

These services provide the security, routing, and organizational context required for all downstream domain services, following Clean Architecture, .NET Aspire orchestration, and TDD principles with ≥80% test coverage.
...
#### Estimated Timeline: 9-10 Weeks (Updated for B2B/B2C and Delegation Features)

- **Week 1-2**: Foundation & Infrastructure Setup (Aspire, ServiceDefaults)
- **Week 3-4**: Identity Service Implementation (Domain + Application layers with B2B/B2C, token exchange, sessions, memberships)
- **Week 5-6**: Identity Service Implementation (Infrastructure + API layers with authorization caching, invitation workflows)
- **Week 6-7**: Configuration Service Implementation (parallel development possible)
- **Week 7-8**: API Gateway Implementation (YARP with tenant-aware routing)
- **Week 9**: Integration Testing (E2E flows, tenant context switching, invitation acceptance)
- **Week 10**: Performance Testing, Security Testing & Documentation

**Timeline Increase Rationale**: Added ~2 weeks for:
- Token exchange pattern implementation and testing
- Session and Membership entity implementation
- Invitation workflow with email integration and verification
- Tenant context switching with cache invalidation
- Authorization cache optimization
- Additional BDD scenarios for delegation workflows
...
- ExternalProviderLink for Entra ID B2B/B2C
...
- Entra ID B2B/B2C link supports both organizational and external identities
- 100% unit test coverage
```

**Task 1.3a: Session Entity with Tenant Context**
```
Duration: 3 hours
TDD: SessionTests.cs → Session.cs

Deliverables:
- Session entity with session ID, user ID, active tenant ID
- Entra token hash storage (SHA256)
- LMS token metadata (expiration, issued timestamp)
- SwitchTenant(targetTenantId) method
- Touch(timestamp) for activity tracking
- Domain events: TenantContextSwitchedEvent

Acceptance:
- Session tracks active tenant for multi-tenant users
- Token hashes stored securely (no plaintext tokens)
- Tenant switching validates target tenant membership
- Activity timestamp updates on each access
- 100% unit test coverage
```

**Task 1.3b: Membership Entity (Tenant-Scoped Roles)**
```
Duration: 4 hours
TDD: MembershipTests.cs → Membership.cs

Deliverables:
- Membership entity (user-tenant-role association)
- AssignmentStatus enumeration (Active, Invited, Revoked)
- InviteSent, VerifiedAt, ExpiresAt timestamps
- Verify() method for invitation acceptance
- Revoke() method for access removal
- Domain events: MembershipInvitedEvent, MembershipVerifiedEvent, MembershipRevokedEvent

Acceptance:
- User can have multiple memberships (different tenants/roles)
- Invitation expiration enforced (7 days from invite)
- Verification updates status to Active
- Revocation immediately prevents access
- 100% unit test coverage
```

**Task 1.3c: Invitation Entity**
```
Duration: 3 hours
TDD: InvitationTests.cs → Invitation.cs

Deliverables:
- Invitation entity with secure token (GUID)
- Target email, tenant, role fields
- InvitationStatus enumeration (Pending, Accepted, Expired, Revoked)
- 10-minute idempotency window logic
- 7-day absolute expiration
- Accept() and Revoke() methods
- Domain events: InvitationSentEvent, InvitationAcceptedEvent

Acceptance:
- Invitation tokens are cryptographically secure
- Duplicate invitations within 10 minutes collapse to single record
- Acceptance within 7 days succeeds; after 7 days fails
- Single-use token pattern (invalidated after first use)
- 100% unit test coverage
```
...
**Task 1.4a: Token Exchange Command (BFF Pattern)**
```
Duration: 6 hours
TDD: ExchangeEntraTokenCommandHandlerTests.cs

Deliverables:
- ExchangeEntraTokenCommand with Entra JWT input
- ExchangeEntraTokenCommandHandler
- ITokenExchangeService interface
- TokenExchangeCommandContext (Entra token, active tenant ID, IP, user agent)
- TokenExchangeResult with LMS custom JWT

Acceptance:
- Validates incoming Entra JWT signature locally
- Exchanges for LMS custom token with tenant context
- LMS token includes userId, tenantId, roles claims
- LMS token expiration set to 1 hour
- Session created/updated with token metadata
- ≥85% test coverage
```

**Task 1.5: Entra ID Authentication (B2B/B2C)**
```
Duration: 10 hours
TDD: EntraIdAuthCommandHandlerTests.cs

Deliverables:
- EntraIdAuthCommand supporting B2B and B2C flows
- EntraIdAuthCommandHandler
- IEntraIdService interface with B2B/B2C configuration
- Microsoft.Identity.Web configuration for both tenant types
- External provider linkage logic
- Auto-user creation for Entra ID B2B/B2C users

Acceptance:
- B2B: Organizational accounts authenticate successfully
- B2C: External identity accounts authenticate successfully
- Entra ID callback creates/updates user with subject ID
- Claims mapped from Entra ID token (both B2B and B2C)
- User linked to ExternalProviderLink with provider type
...
**Task 1.7a: Invitation Workflow Commands**
```
Duration: 8 hours
TDD: InviteUserCommandHandlerTests.cs, AcceptInvitationCommandHandlerTests.cs

Deliverables:
- InviteUserCommand & Handler (System Admin invites District Admin)
- AcceptInvitationCommand & Handler (user accepts via email link)
- ResendInvitationCommand & Handler
- RevokeInvitationCommand & Handler
- IInvitationRepository interface
- IEmailService interface (for sending invites)
- Idempotency logic (10-minute window)

Acceptance:
- Invitation created with secure token and 7-day expiration
- Email sent to target user with verification link
- Duplicate invitations within 10 minutes collapse to single record
- Acceptance creates Membership with Active status
- Expired invitations (>7 days) cannot be accepted
- Resend updates timestamp and sends new email
- ≥85% test coverage
```

**Task 1.7b: Tenant Context Switching Command**
```
Duration: 5 hours
TDD: SwitchTenantContextCommandHandlerTests.cs

Deliverables:
- SwitchTenantContextCommand & Handler
- ISessionRepository interface
- IMembershipRepository interface
- Cache invalidation for previous tenant context

Acceptance:
- Validates user has membership in target tenant
- Updates Session.ActiveTenantId
- Clears authorization cache for previous tenant
...
```
Duration: 5 hours
TDD: GetUserQueryHandlerTests.cs, ListUsersQueryHandlerTests.cs, GetUserTenantsQueryHandlerTests.cs

Deliverables:
- GetUserQuery & Handler
- ListUsersQuery & Handler (paginated)
- GetUserRolesQuery & Handler
- GetUserTenantsQuery & Handler (returns all tenant memberships)
- GetCurrentUserContextQuery & Handler (session + active tenant)
- UserDto, TenantSummaryDto, UserContextDto mappings

Acceptance:
- Users filtered by tenant_id automatically
- Pagination works correctly
- Role and membership information included
- Current user context includes active tenant selection
- ≥80% test coverage
```
...
**Task 1.11: User, Session, Membership, Invitation Repositories**
```
Duration: 8 hours
...
- UserRepository implementing IUserRepository
  - GetByIdAsync, GetByEmailAsync, GetByEntraSubjectIdAsync methods
  - AddAsync, UpdateAsync, DeleteAsync (soft)
- SessionRepository implementing ISessionRepository
  - GetActiveSessionAsync, CreateAsync, UpdateAsync
  - CleanupExpiredSessionsAsync (background job)
- MembershipRepository implementing IMembershipRepository
  - GetByUserAndTenantAsync, GetUserMembershipsAsync
  - CreateAsync, UpdateAsync, RevokeAsync
- InvitationRepository implementing IInvitationRepository
  - GetByTokenAsync, GetPendingInvitationsAsync
  - CreateAsync, UpdateAsync, CleanupExpiredAsync
...
- Invitation cleanup job expires invitations after 7 days
- ≥80% integration test coverage
```

**Task 1.12: Token Service (JWT Generation) and Token Exchange**
```
Duration: 7 hours
TDD: JwtTokenServiceTests.cs, TokenExchangeServiceTests.cs

Deliverables:
- JwtTokenService implementing ITokenService
  - LMS custom JWT generation with claims
  - Token validation
  - Refresh token generation
- TokenExchangeService implementing ITokenExchangeService
  - Validates incoming Entra JWT
  - Issues LMS custom token with tenant context
  - Stores token metadata in session

Acceptance:
- LMS tokens include user_id, tenant_id, roles, membership_ids claims
- LMS tokens expire after 1 hour (configurable)
- Entra JWT validated using cached public keys
- Token exchange completes in <50ms
- Token signature validated
- ≥85% test coverage
```

**Task 1.12a: Authorization Cache Service**
```
Duration: 5 hours
TDD: AuthorizationCacheServiceTests.cs

Deliverables:
- AuthorizationCacheService implementing IAuthorizationCache
- Cache key pattern: authz:{userId}:{tenantId}:{resource}:{action}
- CacheAuthorizationDecisionAsync method
- GetCachedDecisionAsync method
- ClearForUserAndTenantAsync method (invalidation)
- 10-minute TTL with sliding expiration

Acceptance:
- Authorization decisions cached in Redis
- Cache hit returns result in <5ms
- Cache miss triggers decision and caches result
- Invalidation clears all entries for user-tenant combination
- Cache hit rate monitored (target >90%)
- ≥80% test coverage
```

**Task 1.13: Entra ID B2B/B2C Integration**
```
Duration: 10 hours
...
- Microsoft.Identity.Web configuration for B2B and B2C
  - B2B tenant configuration for organizational accounts
  - B2C tenant configuration for external identities (future students/parents)
- Token acquisition for both flows
- Claims transformation (map Entra claims to LMS roles)
- Fallback detection (IsAvailable check)
- IEntraTokenValidator for local JWT validation

Acceptance:
- B2B OAuth flow redirects to Entra ID organizational login
- B2C OAuth flow redirects to Entra B2C (when configured)
- Callback exchanges authorization code for token
- Claims extracted from ID token (both B2B and B2C formats)
- Local JWT validation using cached Entra public keys
...
**Task 1.16: Authentication Controller (with Token Exchange)**
```
Duration: 7 hours
...
- GET /api/v1/auth/login/entra (B2B/B2C redirect)
- GET /api/v1/auth/callback/entra (OAuth callback)
- POST /api/v1/auth/exchange (token exchange: Entra JWT → LMS token)
- POST /api/v1/auth/refresh
- POST /api/v1/auth/logout
- Error handling middleware

Acceptance:
- Local login returns LMS custom JWT
- Entra login redirects to B2B or B2C flow
- Callback exchanges code for Entra token, then exchanges for LMS token
- Token exchange endpoint validates Entra JWT and returns LMS token
...
**Task 1.16a: Invitation and Context Controller**
```
Duration: 6 hours
TDD: BDD scenarios (Reqnroll)

Deliverables:
- InvitationsController (System Admin only)
  - POST /api/v1/invitations (invite user to role)
  - POST /api/v1/invitations/{id}/resend
  - DELETE /api/v1/invitations/{id} (revoke)
  - GET /api/v1/invitations/verify/{token} (public - verify invitation)
- TenantContextController
  - GET /api/v1/context/tenants (get user's available tenants)
  - POST /api/v1/context/switch (switch active tenant)
  - GET /api/v1/context/current (get current tenant context)
- Authorization policies (System Admin for invitations)

Acceptance:
- Invitation endpoints enforce System Admin authorization
- Verification endpoint is public (unauthenticated)
