# Multi-Tenant Database Architecture Pattern

**Pattern**: Database-Per-Service with Row-Level Security Isolation  
**Related Patterns**: [Multi-Tenancy Pattern](./multi-tenancy.md), [Clean Architecture](./clean-architecture.md)  
**Business Value**: Consolidate 100s of district databases into service-specific multi-tenant databases

---

## Overview

The multi-tenant database architecture consolidates per-district databases into a single database per service, with strict tenant isolation enforced through PostgreSQL Row-Level Security (RLS) policies and application-level validation.

**Architecture Goals**:
- Reduce database infrastructure from 100+ district databases to 11 service databases
- Enforce tenant isolation at database and application layers
- Maintain query performance through proper indexing
- Support efficient data migration from legacy per-district architecture
- Enable horizontal scaling and simplified backup/restore operations
...
**Multi-Tenancy**: [Multi-Tenancy Pattern](./multi-tenancy.md) - Application-level tenant isolation  
**Clean Architecture**: [Clean Architecture](./clean-architecture.md) - Domain isolation from infrastructure  
**Caching**: [Caching & Performance](./caching-performance.md) - Per-tenant caching strategies  
**Messaging**: [Messaging Integration](./messaging-integration.md) - Event-driven tenant context propagation
...
**Indexing Strategy**:
```sql
-- Primary access pattern: filter by tenant, sort by created_at
CREATE INDEX idx_students_tenant_created ON student.students (tenant_id, created_at DESC);

-- Search pattern: filter by tenant, search by name
CREATE INDEX idx_students_tenant_name ON student.students (tenant_id, last_name, first_name);

-- Foreign key pattern: filter by tenant, join on school
CREATE INDEX idx_enrollments_tenant_school ON student.enrollments (tenant_id, school_id);
```
...
1. Extract schema from legacy per-district databases
2. Generate unified multi-tenant schema with `tenant_id` columns
3. Create ETL jobs to migrate data with tenant tagging
4. Validate data integrity with reconciliation queries
5. Implement dual-write during transition period
6. Cutover to new database after validation
7. Decommission legacy databases after retention period
