...
- Migrate `IdentityServer` to Duende IdentityServer + Entra ID
...
- Setup dual-authentication (support legacy tokens during transition)
...
3. Setup centralized package management in `Directory.Packages.props`:
   ...
   <Project>
     ...
     <ItemGroup>
       ...
       <PackageVersion Include="Duende.IdentityServer" Version="7.x" />
       <!-- ... -->
     </ItemGroup>
   </Project>
   ```
...
| AspNetUsers, AspNetRoles, AspNetUserClaims | Identity | User, Role, Claim | Migrate to Duende IdentityServer schema |
