...
namespace Northstar.Core
{
            ...
            ICloneable
    {
        ...
        /// <summary>
        /// When web url ends in "/", this causes an issue, remove it
        /// </summary>
        /// <param name="webUrl">Url of the web.</param>
        /// <returns></returns>
        static public string WebUrlFixer(string webUrl)
        {
            if (webUrl.EndsWith("/"))
            {
                return webUrl.Remove(webUrl.Length - 1);
            }
            else
            {
                return webUrl;
        ...
        public object Clone()
        {
            QueryStringHelper dst = new QueryStringHelper(this.BeforeUrl);

           // dst.InternalCurrentPage = this.InternalCurrentPage;
            dst.BeforeUrl = this.BeforeUrl;
            dst.QueryString = this.QueryString;
