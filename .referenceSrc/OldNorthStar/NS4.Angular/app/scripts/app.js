(function () {
    ...
    var mainModule = angular
      ...
      .filter('unsafe', ['$sce', function ($sce) {
        ...
        .filter('inArray', function ($filter) {
            return function (list, arrayFilter, arrayProp, itemId) {
                if (itemId) {
                    return $filter("filter")(list, function (listItem) {
                        var ary = listItem[arrayFilter];
                        for (var i = 0; i < ary.length; i++) {
                            if (ary[i][arrayProp] == itemId) {
                                return true;
                            }
                        }
                        return false;
        ...
        .filter('nsDateFormat', [function () {
            return function (val) {
                ...
                .filter('minutes', [function () {
                    return function (val) {
                        return isNaN(val) ? '0' : (val / 60).toFixed(1);
                    };
                }])
            ...
            .filter('nsTimeOnly', [function () {
                return function (val) {
                    return val ? moment(val).format('h:mm a') : null;
                };
            }])
      ...
      .controller('MainController', ['$scope', '$global', '$timeout', 'progressLoader', '$location', 'authService', '$window', '$http', 'webApiBaseUrl', '$bootbox', 'nsPinesService', '$rootScope', '$uibModal','$route',
          function ($scope, $global, $timeout, progressLoader, $location, authService, $window, $http, webApiBaseUrl, $bootbox, nsPinesService, $rootScope, $uibModal, $route) {
          var mc = self;

          var getInlineCommentsSetting = function () {
              $http.post(webApiBaseUrl + '/api/FilterOptions/GetExistingBoolSetting', { value: 'turnOffCommentBubbles' }).then(function (response) {
                  $scope.userSettings.turnOffInlineComments = response.data.Value;
                  $global.set('turnOffCommentBubbles', response.data.Value);
              });
          }
          ...
          $scope.userSettings = {};
          //$scope.userSettings.turnOffInlineComments = $global.get('turnOffCommentBubbles');
          //$scope.currentUser = nsUserInfo.currentUser;


           
          $scope.showDisplaySettingsModal = function () {
              // set settings
              //$scope.copySettings.copyAll = copyAll;
              //$scope.copySettings.targetResult = studentResult;

              var modalInstance = $uibModal.open({
                  templateUrl: 'changeDisplaySettings.html',
                  scope: $scope,
                  controller: function ($scope, $uibModalInstance) {
                      $scope.toggleCommentBubbles = function () {

                          // store the setting
                          $http.post(webApiBaseUrl + '/api/FilterOptions/UpdateStaffSetting', { SettingName: 'turnOffCommentBubbles', SettingValue: $scope.userSettings.turnOffInlineComments });
                          $global.set('turnOffCommentBubbles', $scope.userSettings.turnOffInlineComments);
          ...
          $scope.$on('$routeChangeStart', function (e) {
              ...
              // only do this if the user is already signed in and we haven't gotten the setting yet
              if (authService.authentication.isAuth && !angular.isDefined($scope.userSettings.turnOffInlineComments)) {
                  getInlineCommentsSetting();
              }

              var resetLinkPath = '/reset-password-from-link';
               
              // don't check on pages where you're not logged in yet and not PRINTMODE
              if (
                  $location.path() !== '/login' &&
                  $location.path() !== '/401' &&
                  $location.absUrl().indexOf('printmode=') < 0 &&
                  ...
                  $http.get(webApiBaseUrl + '/api/PersonalSettings/GetUserCurrentVersion')
                      .then(function (response) {
                          ...
                          // if the version in this file is less than the DB-retrieved version, reload
                          if (($rootScope.appNSVersion < $rootScope.NSVersion) && $rootScope.NSVersion != 0) {
      ...
      .config(['$provide', '$routeProvider', '$locationProvider', '$httpProvider', function ($provide, $routeProvider, $locationProvider, $httpProvider) {
          ...
          .when('/callback/:token',
          {
            ...
            .when('/student-dashboard-printall/:id/:tab?',
              {
                  templateUrl: function (param) { return 'views/student-dashboard-printall.html' }, requireToken: true,
                  resolve: {
                      lookupFields: ['nsLookupFieldService', function (nsLookupFieldService) {
                          return nsLookupFieldService.LoadLookupFields();
                      }]
              ...
              .when('/tm-attend-notes/:teamMeetingId/:studentId',
              {
                ...
                .when('/section-report-avmr-structured-detail/:assessmentId/:sectionId/:benchmarkDateId/:groupId/:altDisplayLabel',
              {
                  templateUrl: function (param) { return 'views/section-report-avmr-structured-detail.html' }, requireToken: true,
                  resolve: {
                      userinfo: ['NSUserInfoService', function (NSUserInfoService) {
                          return NSUserInfoService.loadUserInfo();
                      }],
            ...
            .when('/section-assessment-resultlist-kntc/:assessmentId',
              {
                  templateUrl: function (param) { return 'views/section-assessment-resultlist-kntc.html' }, requireToken: true,
                  resolve: {
                      lookupFields: ['nsLookupFieldService', function (nsLookupFieldService) {
                          return nsLookupFieldService.LoadLookupFields();
                      }],
              ...
              .when('/ig-dashboard/:schoolyear/:school/:interventionist/:interventiongroup/:student/:stint',
              {
                ...
                .when('/ig-dashboard-printall/:schoolyear/:school/:interventionist/:interventiongroup/:student/:stint',
              {
                  templateUrl: function (param) { return 'views/ig-dashboard-printall.html' }, requireToken: true,
                  resolve: {
                      lookupFields: ['nsLookupFieldService', function (nsLookupFieldService) {
                          return nsLookupFieldService.LoadLookupFields();
                      }],
            ...
            .when('/utility-batch-print',
              {
                  templateUrl: function (param) { return 'views/utility-batch-print.html' }, requireToken: true,
                  resolve: {
                      OSAssessments: ['observationSummaryAssessmentFieldChooserSvc', function (observationSummaryAssessmentFieldChooserSvc) {
                          return observationSummaryAssessmentFieldChooserSvc.initialize();
                      }]
                  }// this is loaded twice on this page... optimize this in the future... like 125ms... not gonna bother right now
              })
          ...
          .when('/:templateFile/:assessmentId/:classId', {
              ...
              .when('/:templateFile/:assessmentId/:classId/:benchmarkDateId', {
                  templateUrl: function (param) { return 'views/' + param.templateFile + '.html' }, requireToken: true
                      ...
                      .when('/:templateFile/:assessmentId/:classId/:benchmarkDateId/:studentId/:studentResultId', { // benchmark assessment data entry
                          templateUrl: function (param) { return 'views/' + param.templateFile + '.html' }, requireToken: true,
                          resolve: {
                              lookupFields: ['nsLookupFieldService', function (nsLookupFieldService) {
                                  return nsLookupFieldService.LoadLookupFields();
                    ...
                    .when('/:templateFile/:assessmentId/:interventionGroupId/:studentId/:studentResultId', { // interventiong group data entry
                        templateUrl: function (param) { return 'views/' + param.templateFile + '.html' }, requireToken: true,
                        resolve: {
                            lookupFields: ['nsLookupFieldService', function (nsLookupFieldService) {
                                return nsLookupFieldService.LoadLookupFields();
                    ...
                    .when('/:templateFile/:assessmentId/:classId/:benchmarkDateId/:studentId/:studentResultId/:wordRange?', {
                        templateUrl: function (param) { return 'views/' + param.templateFile + '.html' }, requireToken: true,
                        resolve: {
                            lookupFields: ['nsLookupFieldService', function (nsLookupFieldService) {
                                return nsLookupFieldService.LoadLookupFields();
    ...
    } else if (location.hostname == '10.0.0.208') {  
        document.domain = document.domain;
        mainModule
        .constant("webApiBaseUrl", "http://10.0.0.208:16726")
      .constant("ckEditorSettings", {
          imageUploadUrl: "http://10.0.0.208:16726/api/fileuploader/uploadimages",
          uploadUrl: "http://10.0.0.208:16726/api/fileuploader/uploadimages",
          filebrowserImageUploadUrl: "http://10.0.0.208:16726/api/fileuploader/uploadimagesjsResponse"
      })
      .constant("ckEditorSettingsAdmin", {
          imageUploadUrl: "http://10.0.0.208:16726/api/fileuploader/uploadimagesadmin",
          uploadUrl: "http://10.0.0.208:16726/api/fileuploader/uploadimagesadmin",
          filebrowserImageUploadUrl: "http://10.0.0.208:16726/api/fileuploader/uploadimagesjsResponseadmin"
      })
          .constant('ngAuthSettings', {
              apiServiceBaseUri: "http://10.0.0.208:16725/connect/",
              clientId: 'roclient'
          });
    } 
    else {
