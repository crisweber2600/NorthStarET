(function () {
    ...
    .service('nsTMAssignStudentToIG', ['$http', function ($http) {
        ...
        self.initialize = function (meetingId, meetingName, studentName) {
         ...
         .directive('nsObservationSummaryTmAttend', [
			'$routeParams', '$compile', '$templateCache', '$http', 'nsFilterOptionsService', 'NSObservationSummaryTeamMeetingAttendManager', 'NSSortManager', '$uibModal','nsTMAssignStudentToIG','$location', 'spinnerService', 'nsLookupFieldService',
                ...
                return {
                    ...
                    link: function (scope, element, attr) {
                        ...
                        function LoadData() {
                            ...
                            scope.observationSummaryManager.LoadData(scope.selectedTeamMeetingId, scope.selectedBenchmarkDateId, scope.selectedStaffId)
                                .then(function () { attachFieldsCallback() })
                                .finally(function () { spinnerService.hide('tableSpinner'); });
                        }
    ...
    '$http', 'webApiBaseUrl', function ($http, webApiBaseUrl) {
     ...
     .factory('NSTeamMeetingManager', [
        '$http', '$bootbox', 'nsFilterOptionsService', 'webApiBaseUrl', function ($http, $bootbox, nsFilterOptionsService, webApiBaseUrl) {
            var NSTeamMeetingManager = function () {
                ...
                this.getAttendTeamMeetingList = function (schoolYear) {
                    var returnObject = {
                        SchoolYear: nsFilterOptionsService.normalizeParameter(schoolYear)
                    };

                    return $http.post(webApiBaseUrl + "/api/teammeeting/GetAttendTeamMeetingList", returnObject);
                };
     ...
     .factory('NSTeamMeeting', [
        'nsPinesService', '$http', 'webApiBaseUrl', 'nsFilterOptionsService', function (nsPinesService, $http, webApiBaseUrl, nsFilterOptionsService) {
            var NSTeamMeeting = function (id, flag) {
                ...
                self.initialize = function () {
                    ...
                    meetingData.then(function (response) {
                        ...
                        self.MeetingTime = self.ID > 0 ? moment(self.MeetingTime).toDate() : moment().toDate();
                    }, function (response) {
                        // error callback function
    ...
    function TeamMeetingEditController($scope, $routeParams, $location, nsSectionService, nsFilterOptionsService, nsPinesService, nsSelect2RemoteOptions, NSTeamMeeting, NSTeamMeetingManager, $bootbox) {
        ...
        //    if ($routeParams.id === "-1") {
        //        $scope.meeting.TestDueDateId = nsFilterOptionsService.normalizeParameter($scope.filterOptions.selectedBenchmarkDate);
        //        $scope.meeting.SchoolYear = nsFilterOptionsService.normalizeParameter($scope.filterOptions.selectedSchoolYear);
        //    }
        //}, true);
        ...
        $scope.saveTeamMeeting = function () {
            ...
            // set School Year and Benchmark date ID, since they aren't bound
            $scope.meeting.SchoolYear = $scope.filterOptions.selectedSchoolYear.id;
            $scope.meeting.TestDueDateId = $scope.filterOptions.selectedBenchmarkDate.id;
    ...
    TeamMeetingInvitationController.$inject = ['$scope', '$q', '$http', '$uibModal', 'nsPinesService', '$location', '$filter', '$routeParams', 'NSTeamMeeting', 'progressLoader', 'webApiBaseUrl'];
    ...
    function TeamMeetingInvitationController($scope, $q, $http, $uibModal, nsPinesService, $location, $filter, $routeParams, NSTeamMeeting, progressLoader, webApiBaseUrl) {
        ...
        $scope.sendSelectedInvites = function () {
            ...
            for (var i = 0; i < noticesToSend.length; i++) {
                ...
                $http.post(webApiBaseUrl + '/api/teammeeting/sendtminvitation', postObject).then(
                    function () {
        ...
        $scope.sendSelectedConcluded = function () {
            ...
            for (var i = 0; i < noticesToSend.length; i++) {
                var tma = noticesToSend[i];
                var postObject = { TeamMeetingId: $scope.meeting.ID, StaffId: tma.StaffID };
                $http.post(webApiBaseUrl + '/api/teammeeting/sendtmconcluded', postObject).then(
                    function () {
                        nsPinesService.incrementInterval();
                    }
                );
    ...
    TeamMeetingNotesController.$inject = ['$scope', '$q', '$http', '$uibModal', 'nsPinesService', '$location', '$filter', '$routeParams', 'NSTeamMeeting', '$bootbox', 'NSUserInfoService'];

    function TeamMeetingNotesController($scope, $q, $http, $uibModal, nsPinesService, $location, $filter, $routeParams, NSTeamMeeting, $bootbox, NSUserInfoService) {
        ...
        {
            if(noteStaffId == $scope.currentUser.Id)
            {
                return true;
            } else {
                return false;
    ...
    function TeamMeetingAttendListController($scope, $q, $http, nsPinesService, $location, $filter, $routeParams, nsSectionDataEntryService, nsFilterOptionsService, NSTeamMeetingManager) {
        ...
        $scope.staticColumnsObj.teacherNameHeaderClass = "fa";
        $scope.filterOptions = nsFilterOptionsService.options;
        ...
        var LoadData = function () {
            ...
            if ($scope.filterOptions.selectedSchoolYear != null) {
                $scope.TeamMeetingManager.getAttendTeamMeetingList($scope.filterOptions.selectedSchoolYear)
                    .then(
                        function (data) {
    ...
    TeamMeetingListController.$inject = ['$scope', '$q', '$http', 'nsPinesService', '$location', '$filter', '$routeParams', 'nsSectionDataEntryService', 'nsFilterOptionsService', 'NSTeamMeetingManager', '$bootbox'];

    function TeamMeetingListController($scope, $q, $http, nsPinesService, $location, $filter, $routeParams, nsSectionDataEntryService, nsFilterOptionsService, NSTeamMeetingManager, $bootbox) {
        $scope.sortArray = [];
        $scope.headerClassArray = [];
        $scope.staticColumnsObj = {};
        $scope.staticColumnsObj.studentNameHeaderClass = "fa";
        ...
        $scope.deleteMeeting = function (id) {
            $bootbox.confirm('Are you sure you want to delete this Team Meeting?  <br><br><b>Note: All notes and any other information associated with this meeting will be deleted.</b>',
                function (response) {
                    if (response) {
                        $scope.TeamMeetingManager.delete(id).then(function () {
                            nsPinesService.dataDeletedSuccessfully();
                            LoadData();
                        });
                    }
