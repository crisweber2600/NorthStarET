...
angular
    ...
    .service('nsStaffService', [
			'$http', 'pinesNotifications', 'webApiBaseUrl', function ($http, pinesNotifications, webApiBaseUrl) {
			    ...
			    self.consolidateAccounts = function (primaryStaffId, secondaryStaffId) {
			        return $http.post(webApiBaseUrl + '/api/staff/ConsolidateStaff', { PrimaryStaffId: primaryStaffId, SecondaryStaffId: secondaryStaffId });
			    }
     ...
     .controller('StaffConsolidationController', ['$scope', 'nsPinesService', 'nsSelect2RemoteOptions', '$bootbox', 'progressLoader', 'nsStaffService',
        function ($scope, nsPinesService, nsSelect2RemoteOptions, $bootbox, progressLoader, nsStaffService) {

            $scope.resetForm = function () {
                $scope.moveSettings = { staffPrimary: null, staffSecondary: null, moveComplete: false };
                $scope.warnings = [];
                $scope.errors = [];
            }
            ...
            $scope.checkSameUser = function () {
                if ($scope.moveSettings.staffPrimary === null || $scope.moveSettings.staffSecondary === null) {
                    return false;
                }

                return ($scope.moveSettings.staffPrimary.LastName != $scope.moveSettings.staffSecondary.LastName || $scope.moveSettings.staffPrimary.FirstName != $scope.moveSettings.staffSecondary.FirstName);
            }
            ...
            $scope.quickSearchStaff = nsSelect2RemoteOptions.StaffDetailedQuickSearchRemoteOptions;

            // watch targetSection, show warning if not same year
            $scope.processMove = function () {
                if ($scope.moveSettings.staffPrimary === null || $scope.moveSettings.staffSecondary === null) {
                    $bootbox.alert('You must select two staff accounts to consolidate before continuing.');
                    return;
                }

                $bootbox.confirm('Are you sure you want to consolidate these two accounts? <br> <b>NOTE:</b> THIS IS IRREVERSIBLE.', function (response) {
                    if (response) {
                        // loader
                        ...
                        // do move
                        nsStaffService.consolidateAccounts($scope.moveSettings.staffPrimary, $scope.moveSettings.staffSecondary).then(function (response) {
                            progressLoader.end();
                            // pinesnotifcation
...
StaffListController.$inject = ['$scope', 'nsStaffService', 'nsFilterOptionsService', 'nsSelect2RemoteOptions', '$location', '$bootbox', 'nsPinesService', 'spinnerService', '$timeout'];
...
function StaffListController($scope, nsStaffService, nsFilterOptionsService, nsSelect2RemoteOptions, $location, $bootbox, nsPinesService, spinnerService, $timeout) {
	...
	var LoadData = function (schoolId) {
	    ...
	    if (schoolId > 0) {
	        nsStaffService.getStaffBySchool($scope.filterOptions.selectedSchool.id)
