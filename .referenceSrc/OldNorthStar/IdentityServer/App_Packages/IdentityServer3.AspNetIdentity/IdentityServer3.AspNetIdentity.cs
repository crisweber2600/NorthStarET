...
namespace IdentityServer3.AspNetIdentity
{
        ...
        where TKey : IEquatable<TKey>
    {
        ...
        public static void SendAccountLockedOutEmail(string email)
        {
            try
            {
                ...
                headerText = "The following account has been locked out and is attempting to log in: <b>{0}</b>";

                emailText.AppendFormat(headerText, email);
                ...
                message.Subject = "Locked out account attempting login";
            ...
            if (userManager.SupportsUserPassword)
            {
                ...
                if (user != null)
                {
                    ...
                    if (userManager.SupportsUserLockout &&
                        isLockedOut)
                    {
                        // add a message saying locked out
                        //ctx.SignInMessage = new SignInMessage() { }
                        ctx.AuthenticateResult = new AuthenticateResult("Your account is currently Locked Out.  Please contact your District Administrator.");
                        SendAccountLockedOutEmail(username);
                        return;
                    }
                    ...
                    else if (userManager.SupportsUserLockout)
                    {
                        ...
                        ctx.AuthenticateResult = new AuthenticateResult(String.Format("Incorrect username or password.  You have {0} more attempts before your account is Locked Out.", userManager.MaxFailedAccessAttemptsBeforeLockout - accessFailedCount));
                    }
                }
            }
        }
